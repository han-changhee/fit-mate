...
- [x] `api/routine.ts`에 Gemini API(`gemini-2.5-flash-lite`, 무료 티어) 연동 코드 완성
- [ ] **`GEMINI_API_KEY` 환경변수 미설정** — 로컬(`.env.local`)과 Vercel 양쪽 다 아직 없음. 설정 전까지는 더미 루틴(플랭크/스쿼트 고정)으로 안전하게 폴백 동작 중
