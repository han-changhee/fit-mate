...
- ✅ 서비스 이용약관 · 개인정보처리방침 초안 작성(법률 검토 전)
- ✅ 로그인/회원가입/회원탈퇴 구현(토스 로그인, `appLogin()`) — 단, 서버 사이드 신원 검증은 토스 파트너 콘솔 등록 전이라 미완성
- ⬜ 토스 파트너 콘솔 등록, 광고 슬롯/알림 템플릿 코드/`TOSS_CLIENT_ID`·`TOSS_CLIENT_SECRET` 발급
